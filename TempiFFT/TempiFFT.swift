    ...
    func fftForward(_ inMonoBuffer:[Float]) {
        ...
        if self.windowType != .none {
            ...
            if self.window == nil {
                ...
                case .hamming:
                    vDSP_hamm_window(UnsafeMutablePointer(&self.window), UInt(size), 0)
                case .hanning:
                    vDSP_hann_window(UnsafeMutablePointer(&self.window), UInt(size), Int32(vDSP_HANN_NORM))
                default:
                    break
                }
            }
...
//        let iFloats = [Float](UnsafeBufferPointer(start: self.complexBuffer.imagp, count: self.halfSize))
        ...
        vDSP_fft_zrip(self.fftSetup, UnsafeMutablePointer(&self.complexBuffer), 1, UInt(self.log2Size), Int32(FFT_FORWARD))
        ...
        vDSP_zvmags(UnsafeMutablePointer(&self.complexBuffer), 1, UnsafeMutablePointer(&self.magnitudes), 1, UInt(self.halfSize))
